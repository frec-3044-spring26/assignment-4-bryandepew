...
sunapee|>
  ...
  labs(title = "Ice Off Day by Year", x = "Year", y = "Ice Off Day of Year")+
  geom_point()+
  geom_line()+
  theme_bw()
```
...
I expect the slope of this relationship to be negative since we know the average global temperature has increased over time, meaning the ice off day would be increasingly earlier as this trend progresses.
...
**Answer 3:**
The slope is negative, confirming my expectations explained in answer 2.
...
**Answer 4:**
Multiple R-squared = 0.2055,	Adjusted R-squared = 0.2003. These are not close to a value of 1, meaning our model does not explain much of the variation in the data. From these values, year and ice_off_doy are not strongly related
...
**Answer 6:**
The residuals meet the assumptions of the linear regression and are normally distributed with the distribution roughly centered around zero. There is a left tail, however.
```{r}
ggplot(data = sunapee_5) +
...
The slope value is -0.10602, meaning it would take 66.025 years for the lake has a 1 week change in ice off doy. I think this is meaningful due to the slope's very low p-value which indicates that the slope is significant. This shows a clear trend towards an earlier ice off day each year.
...
**Answer 9:**
I am comfortable with proceeding with this linear model due to the statistical significance of the variables and the consistent findings pointing towards an earlier ice off day as time progresses. Additionally, the normal distribution of the residuals indicates that no assumptions are violated.
**Question 10:**
...
sunapee|>
  ...
  geom_point()+
  geom_line()+
  geom_abline(intercept = coef(fit)[1], slope = coef(fit)[2], color = "green") +
  theme_bw()
```
...
**Answer 11:**
2040 Ice off prediction =  Day 102 (102.67)
Width of confidence interval= 33.6 days

```{r}
"2040_prediction" <- (318.94856 + (-0.10602*2040) )

residuals <-as.numeric(residuals(fit))

con_int<- quantile(residuals, probs = c(0.05, 0.95))

"2040_low" <- (-19.4 + `2040_prediction`)
"2040_high" <- (14.2 + `2040_prediction`)
width <- (116.86776 - 83.26776)
```
...
**Answer 12:**
Firstly, I used ggplot to compare the two datasets and their trends. From this, I saw a slightly steeper slope in the later dataset. To confirm this, I then made linear models of each dataset and analyzed their slopes. The earlier dataset has a slope of -0.10967 and the later has a slope of -0.30239, confirming the trend I saw. I then made a plot of the original data with the slope line of the pre-1970 datset in blue and post-1970 datset in red. Simply put, yes the ice off day of year is changing more quickly in more recent years at Lake Sunapee.
...
ggplot()+
  geom_line(data = sunapee_1, aes(x = year, y = ice_off_doy), color = "blue") +
  geom_line(data = sunapee_2, aes(x = year, y = ice_off_doy), color = "red") 

lm_1 <- lm(ice_off_doy ~ year, data = sunapee_1)

lm_2 <- lm(ice_off_doy ~ year, data = sunapee_2)

#summary(lm_1)
#summary(lm_2)

ggplot()+
  geom_point(data = sunapee_5, aes(x = year, y = ice_off_doy))+
  geom_abline(intercept = coef(lm_1)[1], slope = coef(lm_1)[2], color = "blue") +
  geom_abline(intercept = coef(lm_2)[1], slope = coef(lm_2)[2], color = "red") +
  labs(title = "Slope Lines for Linear Models", x = "Year", y = "Ice Off Day")
```
...
Prompt: Now plot the data for Lake Sunapee (just like above - year on x-axis vs. ice of doy on y axis) but add the line from your regression results (e.g., fit intercept and slope). You will use the [geom_abline](https://ggplot2.tidyverse.org/reference/geom_abline.html).

Used to understand the syntax of the geom_abline function
